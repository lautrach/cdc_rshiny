                ...
                tabsetPanel(
                  tabPanel("Insecticide Mortality", fluid=TRUE, class = "din-next-font", style = "font-family: 'DIN Next', sans-serif;",
                           fluidRow(
                             column(
                               12,
                               htmlOutput("instructions", class = "din-next-font"))
                           ),
                           fluidRow(
                             column(
                               12,
                               HTML("<span style='color: black; font-style: italic;'><b>Step 1:</b> CDC has determined bottle bioassay threshold times and diagnostic doses for several species of mosquitoes. Select the mosquito species and insecticide from the dropdown menus below to set your threshold and dose. (NOTE: If you wish to change the diagnostic time for your calculations, you may do so on the Insecticide Diagnostic Times tab above)</span>"))),
                           fluidRow(
                             column(
                               12,
                             ...
                             column(
                               8,
                               htmlOutput("datacheck", class = "din-next-font")
                             )
                           ),
                           fluidRow(
                             column(
                               6,
                               HTML("<span style='color: black; font-style: italic;'><b>Step 2:</b> Enter the number of dead mosquitos at each time point in each bottle into the table. At the final time point, enter the total number of mosquitoes (dead and alive) in the bottles. It may help in counting to freeze the bottle at the end of the experiment to knock down all the mosquitoes</span>")
                             ),
                             column(
                               5,
                               htmlOutput("recommendation", class = "din-next-font")
                             )
                           ),
                           fluidRow(
                             column(
                               4,
                               span(tags$h4("Data", class = "din-next-thin"), style="color:black; font-family:'DIN Next'; font-style:italic"),
                               matrixInput("sample",
                                           value=m,
                             ...
                             column(
                               8,
                               span(tags$h4("Resistance Plots", class = "din-next-thin"), style="color:black; font-family:'DIN Next'; font-style:italic"),
                               plotOutput("plot"))
                           ),
                           fluidRow(
                             column(
                               2,
                               actionButton("go", "TAKE A SCREENSHOT", class = "btn btn-danger btn-lg din-next-font")
                             ),
                             column( 
                               10,
                               htmlOutput("resistantstate", class = "din-next-font")
                             )
                           )),
...
server <- function(session, input, output) {
  ...
  Datacheck = reactive({
    ...
    if(mortality$Control[(nrow(data)-1)]>0.1){
      paste("<font color=\"#FF0000\"><br>Warning: mortality in control bottle is too high. Please repeat bioassay.")
    }
    else{
  ...
  ResistantState = reactive({
    ...
    if (observed_per > 0.97){
      paste("<font color=\"#FF0000\"><span class='din-next-font'><u><b>SNo resistance detected (population susceptible): </b> Consider baseline mechanism testing (enzymes, molecular assays, or CDC bottle bioassay with inhibitors). <b>Continue monitoring.</b></u></span>")
    }
    else if( (observed_per >= 0.90) || (observed_per <= 0.96)){
      paste("<font color=\"#FF0000\"><span class='din-next-font'><u><b>Developing resistance:</b> Consider mechanism testing (enzymes, molecular assays, or CDC bottle bioassay with inhibitors) and field testing.<b> Rotate insecticide products and implement integrated pest management best practices.</b></u></span>")
    }
    else{
      paste("<font color=\"#FF0000\"><span class='din-next-font'><u><b>Resistant:</b> Consider intensity testing (looking at mortality at 120 minutes or CDC bottle bioassay with 1X, 2X, 5X, and 10X the diagnostic dosage of insecticide ), mechanism testing ( enzymes, molecular assays, or CDC bottle bioassay with inhibitors) and field testing. Avoid this insecticide in this population</u></span>")
    }
  })
